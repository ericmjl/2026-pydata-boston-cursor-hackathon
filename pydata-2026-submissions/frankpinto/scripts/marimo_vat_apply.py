"""One-shot marimo _code_mode driver: stdin to execute-code.sh."""
...
SCOPE_UI = """scope_picker = mo.ui.dropdown(
    options={
...
LOAD_CLEAN = """path = Path("data/OnlineRetail.csv")
raw = pl.read_csv(
    path,
...
MODEL_CF = """df_sorted = df_monthly.sort("month")
...
PLOT = """vat_day = __import__("datetime").datetime(2011, 1, 4)
...
async def main():
    async with cm.get_context() as ctx:
        ...
        ctx.edit_cell("_", code=IMPORTS, hide_code=True)

        hello = next(
            (
                cid
                for cid, c in ctx.cells.items()
                if getattr(c, "name", None) == "hello_markdown"
            ),
            None,
        )
        if hello is not None:
            ctx.delete_cell(hello)

        ctx.create_cell(INTRO_MD, after="_", hide_code=True, name="intro_vat_md")
        ctx.create_cell(
            ASSUMPTIONS_MD, after="intro_vat_md", hide_code=True, name="assumptions_md"
        )
        ctx.create_cell(SCOPE_UI, after="assumptions_md", hide_code=True, name="scope_picker")
        ctx.create_cell(
            LOAD_CLEAN, after="scope_picker", hide_code=True, name="load_and_clean"
        )
        ctx.create_cell(
            MONTHLY, after="load_and_clean", hide_code=True, name="monthly_series"
        )
        ctx.create_cell(
            MODEL_CF, after="monthly_series", hide_code=True, name="counterfactual_model"
        )
        ctx.create_cell(
            VALIDATION,
            after="counterfactual_model",
            hide_code=True,
            name="validation_summary",
        )
        ctx.create_cell(
            PLOT, after="validation_summary", hide_code=True, name="plot_actual_vs_cf"
        )

        for cid in list(ctx.cells.keys()):
            if hello is not None and cid == hello:
                continue
            ctx.run_cell(cid)
